...
# custom dataset
from fairseq.data.rrgen_dataset import RRGenDataset
...
def load_rrgen_dataset(
    ...
    for k in itertools.count():
        ...
        if use_knowledge:
            if not split_exists(split_k, src, tgt, use_knowledge, data_path) and not split_exists(split_k, tgt, src, use_knowledge, data_path):
                raise FileNotFoundError(
                    'Ext {} dataset not found: {} ({})'.format(use_knowledge, split, data_path))
            prefix = os.path.join(
                data_path, '{}.{}-{}.'.format(split_k, src, tgt))
            if dataset_impl != 'raw':
                know_dataset = data_utils.load_indexed_dataset(
                    prefix + use_knowledge, ext_know_dict, dataset_impl)
            else:
                know_dataset = data_utils.load_and_map_simple_dataset(
                    prefix + use_knowledge, ext_know_dict, dataset_impl
                )
        else:
            know_dataset = None
...
@register_task('rrgen_translation_knowldg')
class RRGenTranslationTaskKnowldg(FairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-rating', default=None,
                            help='incorporate rating annotations for input review')
        parser.add_argument('--use-knowledge', default=None,
                            help='incorporate ground knowledge for input review')
        parser.add_argument('--use-length', default=None,
                            help='incorporate length attribute for input review')

    def __init__(self, args, src_dict, tgt_dict, ext_senti_dict, ext_cate_dict, ext_rate_dict, ext_know_dict):
        super().__init__(args)
        ...
        self.ext_know_dict = ext_know_dict
    ...
    def setup_task(cls, args, **kwargs):
        ...
        if args.use_knowledge is not None:
            ext_know_dict = cls.load_normalisation_dictionary(os.path.join(
                paths[0], 'dict.{}.txt'.format(args.use_knowledge)))
            logger.info('[{}] dictionary: {} types'.format(args.use_knowledge,
                                                           len(ext_know_dict)))
        else:
            ext_know_dict = None

        return cls(args, src_dict, tgt_dict, ext_senti_dict, ext_cate_dict, ext_rate_dict, ext_know_dict)

    @staticmethod
    def build_normalisation_dictionary(filenames: List, dict_path: str, senti: bool = False, cate: bool = False, rate: bool = False, know: bool = False):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = load_rrgen_dataset(
            ...
            ext_know_dict=self.ext_know_dict,
